-- 268: NSMQ null-topic remediation part 2/4 (100 exact mappings).
PRAGMA foreign_keys=ON;
CREATE TABLE IF NOT EXISTS _migration_268_expected(q TEXT PRIMARY KEY,s TEXT NOT NULL,r TEXT NOT NULL,t TEXT);
DELETE FROM _migration_268_expected;
WITH source(q,s,r,k) AS (VALUES ('nsmq_chem_pod_013','subj_nsmq_chemistry','problem_of_day','topic_atomic'),
...
INSERT INTO _migration_268_expected
...
CREATE TABLE IF NOT EXISTS _migration_268_guard(valid INTEGER NOT NULL CHECK(valid=1));
DELETE FROM _migration_268_guard;
INSERT INTO _migration_268_guard(valid)
SELECT CASE WHEN
  (SELECT COUNT(*) FROM _migration_268_expected)=100
  AND (SELECT COUNT(*) FROM questions q JOIN _migration_268_expected e ON e.q=q.id)=100
  AND NOT EXISTS (SELECT 1 FROM _migration_268_expected WHERE t IS NULL)
  AND NOT EXISTS (SELECT 1 FROM _migration_268_expected e LEFT JOIN subjects s ON s.id=e.s WHERE s.id IS NULL OR s.exam_type_id IS NOT 'exam_nsmq' OR s.is_active<>1)
  ...
  AND (
    ((SELECT COUNT(*) FROM questions q JOIN _migration_268_expected e ON e.q=q.id
  ...
  AND (SELECT COUNT(*) FROM question_bank_remediation_log l JOIN _migration_268_expected e ON e.q=l.entity_id
...
SELECT '268_nsmq_topic_remediation_part_2','question',e.q,'topic_id',NULL,e.t FROM _migration_268_expected e JOIN questions q ON q.id=e.q WHERE q.topic_id IS NULL;
UPDATE questions SET topic_id=(SELECT e.t FROM _migration_268_expected e WHERE e.q=questions.id)
WHERE topic_id IS NULL AND id IN (SELECT q FROM _migration_268_expected);
DROP TABLE _migration_268_expected;
DROP TABLE _migration_268_guard;
