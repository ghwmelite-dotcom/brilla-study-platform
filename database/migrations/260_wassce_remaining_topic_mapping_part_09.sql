-- 260: WASSCE remaining-topic remediation batch 9/15 (exactly 100 authoritative live rows).
PRAGMA foreign_keys = ON;
CREATE TABLE IF NOT EXISTS _migration_260_guard(valid INTEGER NOT NULL CHECK(valid=1));
DELETE FROM _migration_260_guard;
CREATE TABLE IF NOT EXISTS _migration_260_map(question_id TEXT PRIMARY KEY,topic_id TEXT NOT NULL);
DELETE FROM _migration_260_map;
INSERT INTO _migration_260_map VALUES
...
CREATE TABLE IF NOT EXISTS _migration_260_scope(topic_id TEXT PRIMARY KEY,subject_id TEXT NOT NULL);
DELETE FROM _migration_260_scope;
INSERT INTO _migration_260_scope VALUES
...
INSERT INTO _migration_260_guard(valid)
SELECT CASE WHEN
 (SELECT COUNT(*) FROM _migration_260_map)=100
 AND (SELECT COUNT(*) FROM _migration_260_scope)=16
AND (SELECT COUNT(*) FROM topics WHERE id IN ('topic_eco_development','topic_eco_public_finance','topic_gov_international','topic_gov_public_admin','topic_gov_west_africa_development','topic_wassce_biology_diversity_environment','topic_wassce_biology_life_fundamental_unit','topic_wassce_biology_systems_life','topic_wassce_chem_carbon','topic_wassce_chem_elements','topic_wassce_chem_physical','topic_wassce_core_math_algebra','topic_wassce_core_math_data','topic_wassce_core_math_geometry','topic_wassce_core_math_numbers','topic_wassce_english_literary_devices','topic_wassce_physics_atomic_nuclear','topic_wassce_physics_energy','topic_wassce_physics_fields_electronics','topic_wassce_physics_mechanics_matter'))=20
 ...
 AND NOT EXISTS (SELECT 1 FROM _migration_260_map m LEFT JOIN _migration_260_scope e ON e.topic_id=m.topic_id LEFT JOIN questions q ON q.id=m.question_id LEFT JOIN subjects s ON s.id=q.subject_id LEFT JOIN topics t ON t.id=m.topic_id
 ...
 AND NOT EXISTS (SELECT 1 FROM question_bank_remediation_log l LEFT JOIN _migration_260_map m ON m.question_id=l.entity_id
  WHERE l.migration_id='260_wassce_remaining_topic_mapping_part_09' AND (l.entity_type IS NOT 'question' OR l.field_name IS NOT 'topic_id' OR l.old_value IS NOT NULL OR m.question_id IS NULL OR l.new_value IS NOT m.topic_id))
 AND ((SELECT COUNT(*) FROM question_bank_remediation_log l JOIN questions q ON q.id=l.entity_id JOIN topics t ON t.id=l.new_value
   WHERE l.migration_id IN ('252_wassce_remaining_topic_mapping_part_01','253_wassce_remaining_topic_mapping_part_02','254_wassce_remaining_topic_mapping_part_03','255_wassce_remaining_topic_mapping_part_04','256_wassce_remaining_topic_mapping_part_05','257_wassce_remaining_topic_mapping_part_06','258_wassce_remaining_topic_mapping_part_07','259_wassce_remaining_topic_mapping_part_08') AND l.entity_type='question' AND l.field_name='topic_id' AND l.old_value IS NULL
    AND q.topic_id IS l.new_value AND q.subject_id IS t.subject_id)=800
 ...
 AND (((SELECT COUNT(*) FROM question_bank_remediation_log WHERE migration_id='260_wassce_remaining_topic_mapping_part_09')=0
       AND (SELECT COUNT(*) FROM questions q JOIN _migration_260_map m ON m.question_id=q.id WHERE q.topic_id IS NULL)=100)
   OR ((SELECT COUNT(*) FROM question_bank_remediation_log WHERE migration_id='260_wassce_remaining_topic_mapping_part_09')=100
       AND ((SELECT COUNT(*) FROM questions q JOIN _migration_260_map m ON m.question_id=q.id WHERE q.topic_id IS NULL)=100
         OR (SELECT COUNT(*) FROM questions q JOIN _migration_260_map m ON m.question_id=q.id WHERE q.topic_id IS m.topic_id)=100)))
...
SELECT '260_wassce_remaining_topic_mapping_part_09','question',q.id,'topic_id',q.topic_id,m.topic_id FROM questions q JOIN _migration_260_map m ON m.question_id=q.id WHERE q.topic_id IS NULL;
UPDATE questions SET topic_id=(SELECT m.topic_id FROM _migration_260_map m WHERE m.question_id=questions.id)
WHERE topic_id IS NULL AND id IN (SELECT question_id FROM _migration_260_map);
INSERT INTO _migration_260_guard(valid)
SELECT CASE WHEN
 ...
 AND (SELECT COUNT(*) FROM questions q JOIN _migration_260_map m ON m.question_id=q.id WHERE q.topic_id IS m.topic_id)=100
 AND (SELECT COUNT(*) FROM question_bank_remediation_log l JOIN _migration_260_map m ON m.question_id=l.entity_id WHERE l.migration_id='260_wassce_remaining_topic_mapping_part_09' AND l.entity_type='question' AND l.field_name='topic_id' AND l.old_value IS NULL AND l.new_value IS m.topic_id)=100
 AND NOT EXISTS (SELECT 1 FROM questions q JOIN topics t ON t.id=q.topic_id WHERE q.topic_id IS NOT NULL AND q.subject_id IS NOT t.subject_id)
THEN 1 ELSE 0 END;
DROP TABLE _migration_260_scope;
DROP TABLE _migration_260_map;
DROP TABLE _migration_260_guard;
