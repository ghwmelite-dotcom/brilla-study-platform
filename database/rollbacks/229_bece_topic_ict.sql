-- Roll back 229_bece_topic_ict; the immutable remediation ledger is retained for audit.
PRAGMA foreign_keys = ON;

CREATE TABLE IF NOT EXISTS _rollback_229_guard (valid INTEGER NOT NULL CHECK (valid = 1));
DELETE FROM _rollback_229_guard;

INSERT INTO _rollback_229_guard(valid)
SELECT CASE WHEN
  (SELECT COUNT(*) FROM question_bank_remediation_log
   WHERE migration_id = '229_bece_topic_ict'
...
DROP TABLE _rollback_229_guard;
