-- Roll back 236_bece_topic_social_part_2; the immutable remediation ledger is retained for audit.
PRAGMA foreign_keys = ON;

CREATE TABLE IF NOT EXISTS _rollback_236_guard (valid INTEGER NOT NULL CHECK (valid = 1));
DELETE FROM _rollback_236_guard;

INSERT INTO _rollback_236_guard(valid)
SELECT CASE WHEN
  (SELECT COUNT(*) FROM question_bank_remediation_log
   WHERE migration_id = '236_bece_topic_social_part_2'
     AND entity_type = 'question'
     AND field_name = 'topic_id'
     AND old_value IS NULL) = 80
  AND NOT EXISTS (
    SELECT 1
...
SELECT CASE WHEN
  (SELECT COUNT(*)
   ...
   WHERE l.migration_id = '236_bece_topic_social_part_2'
     AND l.entity_type = 'question'
     AND l.field_name = 'topic_id'
     AND q.topic_id IS l.old_value) = 80
  AND NOT EXISTS (
...
DROP TABLE _rollback_236_guard;
