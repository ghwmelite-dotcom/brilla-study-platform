-- Roll back 225_bece_topic_bdt; the immutable remediation ledger is retained for audit.
PRAGMA foreign_keys = ON;

CREATE TABLE IF NOT EXISTS _rollback_225_guard (valid INTEGER NOT NULL CHECK (valid = 1));
DELETE FROM _rollback_225_guard;

INSERT INTO _rollback_225_guard(valid)
SELECT CASE WHEN
  (SELECT COUNT(*) FROM question_bank_remediation_log
   WHERE migration_id = '225_bece_topic_bdt'
...
DROP TABLE _rollback_225_guard;
