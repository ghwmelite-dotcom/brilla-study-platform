-- Roll back 261_wassce_remaining_topic_mapping_part_10; restore only ledger-proven NULL topic IDs.
PRAGMA foreign_keys = ON;
CREATE TABLE IF NOT EXISTS _rollback_261_guard(valid INTEGER NOT NULL CHECK(valid=1));
DELETE FROM _rollback_261_guard;
CREATE TABLE IF NOT EXISTS _rollback_261_map(question_id TEXT PRIMARY KEY,topic_id TEXT NOT NULL);
DELETE FROM _rollback_261_map;
INSERT INTO _rollback_261_map VALUES
...
CREATE TABLE IF NOT EXISTS _rollback_261_scope(topic_id TEXT PRIMARY KEY,subject_id TEXT NOT NULL);
DELETE FROM _rollback_261_scope;
INSERT INTO _rollback_261_scope VALUES
...
INSERT INTO _rollback_261_guard(valid)
SELECT CASE WHEN
 (SELECT COUNT(*) FROM _rollback_261_map)=100
 AND (SELECT COUNT(*) FROM _rollback_261_scope)=6
 ...
 AND (SELECT COUNT(*) FROM question_bank_remediation_log l JOIN _rollback_261_map m ON m.question_id=l.entity_id WHERE l.migration_id='261_wassce_remaining_topic_mapping_part_10' AND l.entity_type='question' AND l.field_name='topic_id' AND l.old_value IS NULL AND l.new_value IS m.topic_id)=100
 AND NOT EXISTS (SELECT 1 FROM question_bank_remediation_log l LEFT JOIN _rollback_261_map m ON m.question_id=l.entity_id LEFT JOIN questions q ON q.id=l.entity_id LEFT JOIN _rollback_261_scope e ON e.topic_id=m.topic_id LEFT JOIN topics t ON t.id=m.topic_id WHERE l.migration_id='261_wassce_remaining_topic_mapping_part_10' AND (l.entity_type IS NOT 'question' OR l.field_name IS NOT 'topic_id' OR l.old_value IS NOT NULL OR m.question_id IS NULL OR l.new_value IS NOT m.topic_id OR q.id IS NULL OR e.topic_id IS NULL OR q.subject_id IS NOT e.subject_id OR (q.topic_id IS NOT NULL AND (t.id IS NULL OR t.subject_id IS NOT e.subject_id)) OR (q.topic_id IS NOT l.new_value AND q.topic_id IS NOT NULL)))
 AND ((SELECT COUNT(*) FROM questions q JOIN _rollback_261_map m ON m.question_id=q.id WHERE q.topic_id IS m.topic_id)=100
   OR (SELECT COUNT(*) FROM questions q JOIN _rollback_261_map m ON m.question_id=q.id WHERE q.topic_id IS NULL)=100)
 AND (SELECT COUNT(*) FROM question_bank_remediation_log l JOIN questions q ON q.id=l.entity_id WHERE l.migration_id IN ('262_wassce_remaining_topic_mapping_part_11','263_wassce_remaining_topic_mapping_part_12','264_wassce_remaining_topic_mapping_part_13','265_wassce_remaining_topic_mapping_part_14','266_wassce_remaining_topic_mapping_part_15') AND l.entity_type='question' AND l.field_name='topic_id' AND q.topic_id IS l.new_value)=0
THEN 1 ELSE 0 END;
UPDATE questions SET topic_id=NULL
WHERE EXISTS (SELECT 1 FROM question_bank_remediation_log l JOIN _rollback_261_map m ON m.question_id=l.entity_id WHERE l.migration_id='261_wassce_remaining_topic_mapping_part_10' AND l.entity_type='question' AND l.field_name='topic_id' AND l.old_value IS NULL AND l.new_value IS questions.topic_id AND l.entity_id=questions.id);
INSERT INTO _rollback_261_guard(valid)
SELECT CASE WHEN (SELECT COUNT(*) FROM questions q JOIN _rollback_261_map m ON m.question_id=q.id WHERE q.topic_id IS NULL)=100 THEN 1 ELSE 0 END;
DROP TABLE _rollback_261_scope;
DROP TABLE _rollback_261_map;
DROP TABLE _rollback_261_guard;
