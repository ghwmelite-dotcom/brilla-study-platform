-- Roll back 232_bece_topic_rme; the immutable remediation ledger is retained for audit.
PRAGMA foreign_keys = ON;

CREATE TABLE IF NOT EXISTS _rollback_232_guard (valid INTEGER NOT NULL CHECK (valid = 1));
DELETE FROM _rollback_232_guard;

INSERT INTO _rollback_232_guard(valid)
SELECT CASE WHEN
  (SELECT COUNT(*) FROM question_bank_remediation_log
   WHERE migration_id = '232_bece_topic_rme'
...
DROP TABLE _rollback_232_guard;
