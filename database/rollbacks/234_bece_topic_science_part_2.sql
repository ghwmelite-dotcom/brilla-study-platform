-- Roll back 234_bece_topic_science_part_2; the immutable remediation ledger is retained for audit.
PRAGMA foreign_keys = ON;

CREATE TABLE IF NOT EXISTS _rollback_234_guard (valid INTEGER NOT NULL CHECK (valid = 1));
DELETE FROM _rollback_234_guard;

INSERT INTO _rollback_234_guard(valid)
SELECT CASE WHEN
  (SELECT COUNT(*) FROM question_bank_remediation_log
   WHERE migration_id = '234_bece_topic_science_part_2'
...
DROP TABLE _rollback_234_guard;
