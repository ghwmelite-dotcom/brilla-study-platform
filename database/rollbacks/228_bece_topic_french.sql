-- Roll back 228_bece_topic_french; the immutable remediation ledger is retained for audit.
PRAGMA foreign_keys = ON;

CREATE TABLE IF NOT EXISTS _rollback_228_guard (valid INTEGER NOT NULL CHECK (valid = 1));
DELETE FROM _rollback_228_guard;

INSERT INTO _rollback_228_guard(valid)
SELECT CASE WHEN
  (SELECT COUNT(*) FROM question_bank_remediation_log
   WHERE migration_id = '228_bece_topic_french'
...
DROP TABLE _rollback_228_guard;
