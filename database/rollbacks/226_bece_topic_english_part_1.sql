-- Roll back 226_bece_topic_english_part_1; the immutable remediation ledger is retained for audit.
PRAGMA foreign_keys = ON;

CREATE TABLE IF NOT EXISTS _rollback_226_guard (valid INTEGER NOT NULL CHECK (valid = 1));
DELETE FROM _rollback_226_guard;

INSERT INTO _rollback_226_guard(valid)
SELECT CASE WHEN
  (SELECT COUNT(*) FROM question_bank_remediation_log
   WHERE migration_id = '226_bece_topic_english_part_1'
...
DROP TABLE _rollback_226_guard;
