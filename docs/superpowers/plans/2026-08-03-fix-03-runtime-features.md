...
4. **`exam_readiness` upsert (learningpath.ts) — resolved in Task 4, no code change needed.** Verified against migration `062_engagement_features.sql:105-118`: the column is `exam_type TEXT` with `CHECK (exam_type IN ('wassce','bece','nsmq'))` and `UNIQUE(user_id, exam_type, subject_id)` — it stores the SLUG, matching what the upsert binds. It is NOT the phantom-era `exam_type_id` shape. Two squash caveats: (a) `exam_readiness` exists ONLY in migration 062 — it is absent from `database/schema.sql`, so the squashed schema must include it or the readiness endpoint breaks; (b) if the squash ever normalizes this to an id FK, the upsert and its `ON CONFLICT(user_id, exam_type, subject_id)` clause must be updated together.
