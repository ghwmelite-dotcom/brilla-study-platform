...
const migrationNames = [
  '593_igcse_chemistry_sprint3_foundation.sql',
  '594_igcse_chemistry_sprint3_part_1.sql',
  '595_igcse_chemistry_sprint3_part_2.sql',
  '596_igcse_chemistry_sprint3_part_3.sql',
  '597_igcse_chemistry_sprint3_part_4.sql',
  '598_igcse_chemistry_sprint3_part_5.sql',
  '599_igcse_chemistry_sprint3_part_6.sql',
  '600_igcse_chemistry_sprint3_part_7.sql',
  '601_igcse_chemistry_sprint3_part_8.sql',
  '602_igcse_chemistry_sprint3_part_9.sql',
  '603_igcse_chemistry_sprint3_part_10.sql',
  '604_igcse_chemistry_sprint3_part_11.sql',
  '605_igcse_chemistry_sprint3_part_12.sql',
  '606_igcse_chemistry_sprint3_part_13.sql',
  '607_igcse_chemistry_sprint3_final_guard.sql',
] as const;

const repoRoot = fileURLToPath(new URL('../../../', import.meta.url));
const migrationRelativePaths = migrationNames.map((name) => `database/migrations/${name}`);
const artifactRelativePaths = ['content/batches/igcse-chemistry-sprint3-001.json', ...migrationRelativePaths];
...
// against brilla-db on 2026-09-10). The fixture seeds them directly so the
// foundation migration's INSERT OR IGNORE re-assertion no-ops, exactly as it
// does on prod and on fresh baselines (prod patch 096).
const seededTopics: Array<[string, string]> = [
  ['topic_igcse_chem_atomic', 'subj_igcse_chemistry'],
  ['topic_igcse_chem_periodic', 'subj_igcse_chemistry'],
  ['topic_igcse_chem_bonding', 'subj_igcse_chemistry'],
  ['topic_igcse_chem_stoichiometry', 'subj_igcse_chemistry'],
  ['topic_igcse_chem_reactions', 'subj_igcse_chemistry'],
  ['topic_igcse_chem_acids', 'subj_igcse_chemistry'],
  ['topic_igcse_chem_metals', 'subj_igcse_chemistry'],
  ['topic_igcse_chem_organic', 'subj_igcse_chemistry'],
  ['topic_igcse_chem_electrolysis', 'subj_igcse_chemistry'],
  ['topic_igcse_chem_analysis', 'subj_igcse_chemistry'],
];
...
function createFixture() {
  ...
  db.exec(`
    ...
    INSERT INTO subjects(id, exam_type_id) VALUES ('subj_igcse_chemistry', 'igcse');
  `);
...
describe('IGCSE Chemistry sprint 3 automated-beta migrations', () => {
  ...
  it('reproduces every committed JSON and SQL artifact byte for byte', () => {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'brilla-igcse-chemistry-sprint3-'));
    try {
      execFileSync(process.execPath, [
        join(repoRoot, 'scripts/generate-igcse-chemistry-sprint3-bank.mjs'),
        '--output-root',
        temporaryRoot,
  ...
  it('loads 50 original questions across ten prod-canonical topics with beta provenance', () => {
    ...
    expect(db.prepare(`
      SELECT COUNT(*) AS count FROM questions WHERE id LIKE 'q_igcse_chem_s3_%'
    `).get()).toEqual({ count: 50 });

    expect(db.prepare(`
      SELECT topic_id AS topicId, COUNT(*) AS count
      FROM questions WHERE id LIKE 'q_igcse_chem_s3_%'
      GROUP BY topic_id ORDER BY topic_id
    `).all()).toEqual([
      { topicId: 'topic_igcse_chem_acids', count: 5 },
      { topicId: 'topic_igcse_chem_analysis', count: 5 },
      { topicId: 'topic_igcse_chem_atomic', count: 5 },
      { topicId: 'topic_igcse_chem_bonding', count: 5 },
      { topicId: 'topic_igcse_chem_electrolysis', count: 5 },
      { topicId: 'topic_igcse_chem_metals', count: 5 },
      { topicId: 'topic_igcse_chem_organic', count: 5 },
      { topicId: 'topic_igcse_chem_periodic', count: 5 },
      { topicId: 'topic_igcse_chem_reactions', count: 5 },
      { topicId: 'topic_igcse_chem_stoichiometry', count: 5 },
    ]);
    ...
    `).all()).toEqual([
      ...
      { questionType: 'structured', count: 5 },
    ]);
    ...
    expect(db.prepare(`
      ...
      FROM questions WHERE id LIKE 'q_igcse_chem_s3_%' AND question_type = 'multiple_choice'
      GROUP BY correct_answer ORDER BY correct_answer
    `).all()).toEqual([
      { correctAnswer: 'A', count: 11 },
      { correctAnswer: 'B', count: 11 },
      { correctAnswer: 'C', count: 11 },
      { correctAnswer: 'D', count: 10 },
    ]);
    ...
    expect(db.prepare(`
      ...
      WHERE q.id LIKE 'q_igcse_chem_s3_%'
        AND q.subject_id = 'subj_igcse_chemistry'
        ...
        AND (
          ...
          OR
          (q.question_type = 'structured' AND q.options IS NULL AND length(q.correct_answer) >= 1
            AND q.points = 4 AND q.marks = 4 AND q.time_limit = 300)
        )
        AND qcr.batch_id = 'igcse-chemistry-sprint3-001'
    ...
    expect(db.prepare(`
      ...
      WHERE batch_id = 'igcse-chemistry-sprint3-001'
    ...
    applyAll(db);
    expect(db.prepare("SELECT COUNT(*) AS count FROM questions WHERE id LIKE 'q_igcse_chem_s3_%'").get()).toEqual({ count: 50 });
    expect(db.prepare("SELECT COUNT(*) AS count FROM question_content_releases WHERE batch_id = 'igcse-chemistry-sprint3-001'").get()).toEqual({ count: 50 });
    expect(db.pragma('foreign_key_check')).toEqual([]);
    db.close();
  });
  ...
  it('fails closed when an expected stable question ID contains different canonical content', () => {
    ...
    db.prepare(`
      ...
      VALUES ('q_igcse_chem_s3_001', 'topic_igcse_chem_atomic', 'subj_igcse_chemistry', 'igcse',
        'Corrupted content under a reserved stable ID', 'multiple_choice', 'fixture', 'easy')
    `).run();
    expect(() => db.exec(migrations[1])).toThrow();
    expect(db.prepare('SELECT question_text AS text FROM questions WHERE id = ?').get('q_igcse_chem_s3_001')).toEqual({
  ...
  it('fails closed when a canonical row acquires false official-paper metadata', () => {
    ...
    db.prepare("UPDATE questions SET source_paper_code = '0620/22', source_question_number = '1' WHERE id = 'q_igcse_chem_s3_001'").run();
    expect(() => db.exec(migrations[1])).toThrow();
    db.close();
  });
  ...
  it('fails closed when existing release provenance differs', () => {
    ...
    db.exec(migrations[0]);
    db.exec(migrations[1]);
    db.prepare("UPDATE question_content_releases SET batch_id = 'wrong-batch' WHERE question_id = 'q_igcse_chem_s3_001'").run();
    expect(() => db.exec(migrations[1])).toThrow();
    db.close();
  });
  ...
  it('fails closed when a reserved ID is pre-bound to the wrong topic of the same subject', () => {
    ...
    db.prepare(`
      INSERT INTO questions (id, topic_id, subject_id, exam_type_id, question_text, question_type, correct_answer, difficulty)
      VALUES ('q_igcse_chem_s3_001', 'topic_igcse_chem_bonding', 'subj_igcse_chemistry', 'igcse',
        'An atom of carbon-14 has proton number 6 and nucleon number 14. How many neutrons does the atom contain?', 'multiple_choice', 'A', 'medium')
    `).run();
    expect(() => db.exec(migrations[1])).toThrow();
    db.close();
  });
  ...
  it('rejects cross-subject topic bindings through the subject/topic consistency trigger', () => {
    ...
    expect(() => db.prepare(`
      INSERT INTO questions (id, topic_id, subject_id, exam_type_id, question_text, question_type, correct_answer, difficulty)
      VALUES ('q_scratch_001', 'topic_algebra', 'subj_igcse_chemistry', 'igcse',
  ...
  it('fails the final guard when earlier question parts are missing', () => {
    ...
    expect(db.prepare("SELECT COUNT(*) AS count FROM questions WHERE id LIKE 'q_igcse_chem_s3_%'").get()).toEqual({ count: 0 });
    db.close();
  });
});
