...
function validBatch() {
  return {
    batchId: 'pilot-001', status: 'approved_for_staging', examTypeId: 'edexcel_igcse',
    provenance: [{ publisher: 'Pearson', title: 'Official specification', url: 'https://example.com/spec.pdf', use: 'curriculum_blueprint_only' }],
    review: { authoringMethod: 'original_curriculum_aligned', academicReviewer: 'Reviewer', reviewedAt: '2026-08-24T00:00:00Z' },
...
describe('question content validation', () => {
  ...
  it('rejects duplicate text and unreviewed production content', () => {
    const batch = validBatch();
    batch.status = 'approved_for_production';
    batch.review.academicReviewer = null;
    batch.review.reviewedAt = null;
    ...
    expect(result.errors.join('\n')).toContain('academicReviewer');
    expect(result.errors.join('\n')).toContain('at least 40');
  });
  it('refuses an import manifest from an unapproved draft', () => {
    const batch = validBatch();
    batch.status = 'draft_academic_review';
    expect(() => buildStagingManifest(batch)).toThrow('approved_for_staging');
  });
});
