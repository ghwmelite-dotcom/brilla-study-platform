...
const subjectDefinitions = [
  { key: 'music', id: 'subj_wassce_music', firstMigration: 293 },
  { key: 'clerical', id: 'subj_wassce_clerical', firstMigration: 301 },
  { key: 'typewriting', id: 'subj_wassce_typewriting', firstMigration: 309 },
  { key: 'home_mgmt', id: 'subj_wassce_home_mgmt', firstMigration: 317 },
  { key: 'textiles', id: 'subj_wassce_textiles', firstMigration: 325 },
  { key: 'picture_making', id: 'subj_wassce_picture_making', firstMigration: 333 },
  { key: 'fante', id: 'subj_wassce_fante', firstMigration: 341 },
  { key: 'ga', id: 'subj_wassce_ga', firstMigration: 349 },
];
const migrationNames = [
  '284_wassce_foundation_beta_specifications.sql',
  '285_music_beta_foundation.sql',
  '286_clerical_beta_foundation.sql',
  '287_typewriting_beta_foundation.sql',
  '288_home_mgmt_beta_foundation.sql',
  '289_textiles_beta_foundation.sql',
  '290_picture_making_beta_foundation.sql',
  '291_fante_beta_foundation.sql',
  '292_ga_beta_foundation.sql',
  ...subjectDefinitions.flatMap(({ key, firstMigration }) => Array.from(
    { length: 8 },
    (_, index) => `${firstMigration + index}_${key}_beta_part_${index + 1}.sql`,
  )),
  '357_wassce_foundation_beta_final_guard.sql',
];
const repoRoot = fileURLToPath(new URL('../../../', import.meta.url));
const migrationRelativePaths = migrationNames.map((name) => `database/migrations/${name}`);
const artifactRelativePaths = ['content/batches/wassce-foundation-beta-005.json', ...migrationRelativePaths];
...
function createFixture() {
  ...
  db.exec(`
    ...
    INSERT INTO subjects(id, exam_type_id) VALUES
      ('subj_wassce_music', 'exam_wassce'),
      ('subj_wassce_clerical', 'exam_wassce'),
      ('subj_wassce_typewriting', 'exam_wassce'),
      ('subj_wassce_home_mgmt', 'exam_wassce'),
      ('subj_wassce_textiles', 'exam_wassce'),
      ('subj_wassce_picture_making', 'exam_wassce'),
      ('subj_wassce_fante', 'exam_wassce'),
      ('subj_wassce_ga', 'exam_wassce');
  `);
  return db;
}
...
describe('WASSCE foundation automated-beta batch 005 migrations', () => {
  ...
  it('reproduces every committed JSON and SQL artifact byte for byte', () => {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'brilla-wassce-foundation-'));
    try {
      execFileSync(process.execPath, [
        join(repoRoot, 'scripts/generate-wassce-foundation-beta-bank.mjs'), '--output-root', temporaryRoot,
  ...
  it('contains forty unique original MCQs across eight exactly assigned topics per subject', () => {
    expect(batch.batchId).toBe('wassce-foundation-beta-005');
    ...
    expect(batch.provenance.length).toBeGreaterThanOrEqual(24);
    expect(batch.provenance.every((source) => ['West African Examinations Council, Ghana', 'National Council for Curriculum and Assessment, Ghana'].includes(source.publisher))).toBe(true);
    expect(new Set(batch.provenance.map((source) => source.publisher))).toEqual(new Set(['West African Examinations Council, Ghana', 'National Council for Curriculum and Assessment, Ghana']));
    ...
    expect(allPrompts.size).toBe(320);
  });

  it('loads and reapplies 320 relationship-valid questions with exact beta provenance', () => {
    const db = createFixture();
    applyAll(db);
    expect(db.prepare(`
      SELECT subject_id AS subjectId, COUNT(*) AS count FROM questions
      WHERE id LIKE 'q_was_%_b005_%' GROUP BY subject_id ORDER BY subject_id
    `).all()).toEqual([
      { subjectId: 'subj_wassce_clerical', count: 40 },
      { subjectId: 'subj_wassce_fante', count: 40 },
      { subjectId: 'subj_wassce_ga', count: 40 },
      { subjectId: 'subj_wassce_home_mgmt', count: 40 },
      { subjectId: 'subj_wassce_music', count: 40 },
      { subjectId: 'subj_wassce_picture_making', count: 40 },
      { subjectId: 'subj_wassce_textiles', count: 40 },
      { subjectId: 'subj_wassce_typewriting', count: 40 },
    ]);
    expect(db.prepare(`
      ...
      WHERE q.id LIKE 'q_was_%_b005_%' AND q.subject_id = t.subject_id
        ...
        AND r.batch_id = 'wassce-foundation-beta-005' AND r.quality_assurance = 'automated_beta'
    ...
    applyAll(db);
    expect(db.prepare("SELECT COUNT(*) AS count FROM question_content_releases WHERE batch_id = 'wassce-foundation-beta-005'").get()).toEqual({ count: 320 });
    db.close();
  });
  ...
  it('fails closed on a stable-ID collision with different canonical content', () => {
    ...
    db.prepare(`INSERT INTO questions (
      ...
      .run('q_was_music_b005_001', 'topic_was_music_b005_1', 'subj_wassce_music', 'exam_wassce',
        'Corrupted content under a reserved foundation batch identifier', 'multiple_choice',
        ...
        'easy', 1, 1, 90, 'st_was_music_b005_1', 'Identify', 'AO1', 'board_waec');
    expect(() => db.exec(migrations[9])).toThrow();
    db.close();
  });

  it('fails the final guard for missing parts or altered provenance', () => {
    const incomplete = createFixture();
    for (const migration of migrations.slice(0, 9)) incomplete.exec(migration);
    expect(() => incomplete.exec(migrations.at(-1)!)).toThrow();
    incomplete.close();

    const altered = createFixture();
    applyAll(altered);
    altered.prepare("UPDATE question_content_releases SET official_exam_board_content = 1 WHERE question_id = 'q_was_music_b005_001'").run();
