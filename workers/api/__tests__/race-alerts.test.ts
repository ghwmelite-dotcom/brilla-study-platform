...
function stubTelegramFetch(routes: Record<string, { status?: number; body?: unknown }> = {}) {
...
function sentMessages(fetchMock: ReturnType<typeof vi.fn>): { chat_id: string; text: string }[] {
...
function rateLimitHandlers(totalFor: (endpoint: string) => number = () => 0): MockHandler[] {
  return [
    {
      match: /SUM\(request_count\) as total_requests/,
      first: (binds) => ({
        total_requests: totalFor(String(binds[1])),
...
function alertsDb(extra: MockHandler[], totalFor?: (endpoint: string) => number) {
  return createMockD1([
    ...extra,
    { match: /UPDATE race_cycles SET winner_announced_at/, run: () => ({ success: true, meta: { changes: 1 } }) },
    { match: /UPDATE race_cycles SET start_announced_at/, run: () => ({ success: true, meta: { changes: 1 } }) },
...
describe('(a) winner announcement — member gate passes', () => {
  it('names the winner in the platform post, sets winner_announced_at, never reposts', async () => {
    ...
    const mark = db.calls.find((c) => /UPDATE race_cycles SET winner_announced_at/.test(c.sql));
...
describe('(b) winner announcement — winner is NOT a channel member', () => {
  it('post omits the name and the winner gets a join-the-channel DM', async () => {
    ...
    const db = alertsDb([
      { match: /winner_announced_at IS NULL/, all: () => ({ results: [CROWNED_PLATFORM] }) },
      { match: /FROM telegram_links WHERE user_id/, first: () => ({ chat_id: '777' }) },
    ]);
    ...
    expect(db.calls.some((c) => /UPDATE race_cycles SET winner_announced_at/.test(c.sql))).toBe(true);
  });
});

describe('(c) school-scoped winner announcement', () => {
  const CROWNED_SCHOOL = { ...CROWNED_PLATFORM, id: 'cyc_s', scope: 'school', school_id: 'sch_1', winner_user_id: null, winner_name: null };

  it('posts to the school channel when a school_channels row exists', async () => {
    const fetchMock = stubTelegramFetch();
    const db = alertsDb([
      { match: /winner_announced_at IS NULL/, all: () => ({ results: [CROWNED_SCHOOL] }) },
...
describe('(d) cycle-start post', () => {
  it('posts once for a freshly opened cycle and sets start_announced_at', async () => {
    ...
    const db = alertsDb([
      {
        ...
        all: () => ({
          results: [{
            id: 'cyc_new', scope: 'platform', school_id: null,
            starts_at: '2026-08-10 00:00:00', ends_at: '2026-08-17 00:00:00',
          }],
        }),
      },
    ]);
    ...
    const mark = db.calls.find((c) => /UPDATE race_cycles SET start_announced_at/.test(c.sql));
    expect(mark).toBeDefined();
...
const ACTIVE_CYCLE = {
  id: 'cyc_a', scope: 'platform', school_id: null,
  starts_at: '2026-08-10 00:00:00', ends_at: '2026-08-17 00:00:00', target_points: 1000,
};
...
function positionDb(stateForU5: { last_rank: number; last_score: number; alerted_flags: number }) {
  return alertsDb([
    ...
    { match: /INSERT INTO race_alert_state/, run: () => ({ success: true, meta: { changes: 1 } }) },
    { match: /FROM telegram_links WHERE user_id/, first: () => ({ chat_id: '555' }) },
  ]);
}
...
describe('(e) position-passed DM', () => {
  ...
  it('flag already set → no DM, but rank still tracked', async () => {
    const fetchMock = stubTelegramFetch();
    const db = positionDb({ last_rank: 3, last_score: 650, alerted_flags: ALERT_FLAG_POSITION_PASSED });
...
describe('(f) ending-soon DM', () => {
  const ENDING_CYCLE = { ...ACTIVE_CYCLE, id: 'cyc_e', ends_at: '2026-08-12 20:00:00' }; // 5h left

  function endingDb(state: unknown) {
    return alertsDb([
      { match: /rc\.target_points/, all: () => ({ results: [ENDING_CYCLE] }) },
      { match: /SUM\(pl\.points\) AS score/, all: () => ({ results: [RANKED[0]] }) },
      { match: /FROM race_alert_state WHERE user_id/, first: () => state },
      { match: /INSERT INTO race_alert_state/, run: () => ({ success: true, meta: { changes: 1 } }) },
      { match: /FROM telegram_links WHERE user_id/, first: () => ({ chat_id: '111' }) },
    ]);
  }
  ...
  it('ends_at within 12h and flag unset → DM + ALERT_FLAG_ENDING_SOON', async () => {
    ...
    const upsert = db.calls.find((c) => /INSERT INTO race_alert_state/.test(c.sql));
    expect(upsert!.binds[4] as number).toBe(ALERT_FLAG_ENDING_SOON);
  });

  it('flag already set → no DM', async () => {
    const fetchMock = stubTelegramFetch();
    const db = endingDb({ last_rank: 1, last_score: 900, alerted_flags: ALERT_FLAG_ENDING_SOON });
...
describe('(g) streak-rescue DM', () => {
  ...
  const streakHandlers: MockHandler[] = [
    ...
    { match: /FROM telegram_links WHERE user_id/, first: () => ({ chat_id: '321' }) },
  ];
  ...
  it('linked user 22h+ idle → one DM gated by both notify-streak and notify buckets', async () => {
    ...
    expect(STREAK_RESCUE_LIMIT).toEqual({ maxRequests: 1, windowMs: 24 * 60 * 60 * 1000 });
    expect(NOTIFY_DM_LIMIT.maxRequests).toBe(3);
  });

  it('notify-streak bucket already used today → no DM', async () => {
    const fetchMock = stubTelegramFetch();
    const db = alertsDb(streakHandlers, (endpoint) => (endpoint === 'notify-streak' ? 1 : 0));
...
describe('(h) Telegram outage', () => {
  it('fetch throws everywhere → resolves with zero counts, logs, still marks announced', async () => {
    ...
    const db = alertsDb([
      { match: /winner_announced_at IS NULL/, all: () => ({ results: [CROWNED_PLATFORM] }) },
      { match: /FROM telegram_links WHERE user_id/, first: () => ({ chat_id: '777' }) },
      { match: /u\.streak_days > 0/, all: () => ({ results: [{ id: 'u_s', streak_days: 3 }] }) },
    ]);

    await expect(runTelegramRaceAlerts(db, envFor(db), NOW)).resolves.toEqual({ posts: 0, dms: 0 });
