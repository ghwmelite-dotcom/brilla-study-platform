...
function sendMessageChats(fetchMock: ReturnType<typeof vi.fn>): string[] {
  return fetchMock.mock.calls
    ...
    .map(([, init]) =>
      String(JSON.parse((init as RequestInit).body as string).chat_id),
    );
}
...
describe("notifyUser (a) — unlinked user", () => {
  it("no telegram_links row → false, no Telegram fetch", async () => {
    ...
    const sent = await notifyUser(
      db as never,
      { DB: db as never, TELEGRAM_BOT_TOKEN: BOT_TOKEN },
      "user_1",
      "hi",
    );
...
describe("notifyUser (b) — linked user", () => {
  it("sends to the stored chat_id, returns true, records the notify rate-limit slot", async () => {
    ...
    const record = db.calls.find(
      (c) =>
        /INSERT INTO rate_limits|UPDATE rate_limits/.test(c.sql) &&
        c.binds.includes("notify"),
    );
...
describe("notifyUser (c) — daily DM budget exhausted", () => {
  it("4th DM in 24h (rate_limits sum = 3) → false, no Telegram fetch, no extra record", async () => {
    ...
    const sent = await notifyUser(
      exhausted as never,
      { DB: exhausted as never, TELEGRAM_BOT_TOKEN: BOT_TOKEN },
      "user_1",
      "hi",
    );
    expect(sent).toBe(false);
    expect(fetchMock).not.toHaveBeenCalled();
    expect(
      exhausted.calls.filter((call) =>
        call.sql.includes("WITH usage(total_requests)"),
      ),
    ).toHaveLength(1);
  });
});
...
describe("notifyUser (d) — user blocked the bot", () => {
  it("403 from Telegram → marks the link stale, returns false", async () => {
    ...
    const db = notifyDb([
      ...
      {
        match: /UPDATE telegram_links SET stale = 1/,
    ...
    const staleUpdate = db.calls.find((c) =>
      /UPDATE telegram_links SET stale = 1/.test(c.sql),
    );
  ...
  it("non-403 failure → false, link left untouched", async () => {
    ...
    expect(sent).toBe(false);
    expect(
      db.calls.some((c) => /UPDATE telegram_links SET stale/.test(c.sql)),
    ).toBe(false);
  });
});
  ...
  for (const status of [400, 403]) {
    it(`${status} → marks channel broken, notifies every admin with /admin/schools link`, async () => {
      ...
      const db = notifyDb([
        ...
        {
          match: /UPDATE school_channels SET broken = 1/,
          run: () => ({ success: true, meta: { changes: 1 } }),
        },
        {
      ...
      const sent = await notifySchoolChannel(
        db as never,
        { DB: db as never, TELEGRAM_BOT_TOKEN: BOT_TOKEN },
        "school_9",
        "post",
      ...
      const brokenUpdate = db.calls.find((c) =>
        /UPDATE school_channels SET broken = 1/.test(c.sql),
      );
      ...
      expect(notifs.map((c) => c.binds[1]).sort()).toEqual([
        "admin_1",
        "admin_2",
      ]);
...
describe("notifyPlatformChannel (f) — best-effort platform posts", () => {
  it("TELEGRAM_PLATFORM_CHANNEL_ID unset → false, no Telegram fetch", async () => {
    ...
    const sent = await notifyPlatformChannel(
      { DB: createMockD1([]) as never, TELEGRAM_BOT_TOKEN: BOT_TOKEN },
      "post",
    );
    expect(sent).toBe(false);
    expect(fetchMock).not.toHaveBeenCalled();
  ...
  it("posts to the configured channel → true", async () => {
    ...
    const sent = await notifyPlatformChannel(
      {
        DB: createMockD1([]) as never,
        TELEGRAM_BOT_TOKEN: BOT_TOKEN,
        TELEGRAM_PLATFORM_CHANNEL_ID: "-100999",
      },
      "post",
    );
    expect(sent).toBe(true);
