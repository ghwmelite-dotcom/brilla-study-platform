...
import { visualArtsSubjects } from '../../../scripts/wassce-visual-arts-beta-data.mjs';

const subjectDefinitions = [
  { key: 'basketry', id: 'subj_wassce_basketry', firstMigration: 193 },
  { key: 'leatherwork', id: 'subj_wassce_leatherwork', firstMigration: 203 },
  { key: 'sculpture', id: 'subj_wassce_sculpture', firstMigration: 213 },
];
const migrationNames = [
  '189_wassce_visual_arts_beta_foundation.sql',
  '190_basketry_beta_foundation.sql',
  '191_leatherwork_beta_foundation.sql',
  '192_sculpture_beta_foundation.sql',
  ...subjectDefinitions.flatMap(({ key, firstMigration }) => Array.from(
    { length: 10 },
    (_, index) => `${firstMigration + index}_${key}_beta_part_${index + 1}.sql`,
  )),
  '223_wassce_visual_arts_beta_final_guard.sql',
];
const repoRoot = fileURLToPath(new URL('../../../', import.meta.url));
const migrationRelativePaths = migrationNames.map((name) => `database/migrations/${name}`);
const artifactRelativePaths = ['content/batches/wassce-visual-arts-beta-004.json', ...migrationRelativePaths];
...
function createFixture() {
  ...
  db.exec(`
    ...
    INSERT INTO subjects(id, exam_type_id) VALUES
      ('subj_wassce_basketry', 'exam_wassce'),
      ('subj_wassce_leatherwork', 'exam_wassce'),
      ('subj_wassce_sculpture', 'exam_wassce');
  `);
  return db;
}
...
describe('WASSCE visual arts automated-beta batch 004 migrations', () => {
  ...
  it('reproduces every committed JSON and SQL artifact byte for byte', () => {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'brilla-wassce-visual-arts-'));
    try {
      execFileSync(process.execPath, [
        join(repoRoot, 'scripts/generate-wassce-visual-arts-beta-bank.mjs'), '--output-root', temporaryRoot,
  ...
  it('contains forty unique original MCQs across eight exactly assigned topics per subject', () => {
    expect(batch.batchId).toBe('wassce-visual-arts-beta-004');
    ...
    expect(new Set(batch.provenance.map((source) => source.url)).size).toBe(batch.provenance.length);
    expect(new Set(batch.provenance.map((source) => source.publisher))).toEqual(new Set([
      'West African Examinations Council, Ghana',
      'National Council for Curriculum and Assessment, Ghana',
    ]));
    expect(batch.provenance.every((source) => source.url.startsWith('https://waecgh.org/')
      || source.url.startsWith('https://nacca.gov.gh/'))).toBe(true);
    expect(batch.provenance.every((source) => source.use === 'curriculum_blueprint_only')).toBe(true);
    expect(batch.provenance.find((source) => source.url.endsWith(
      'CHIEF-EXAMINERS-REPORTS.-WASSCE-SC-2023.pdf',
    ))?.title).toBe('WASSCE 2023 Chief Examiners Reports');
    const allPrompts = new Set<string>();
    for (const subject of batch.subjects) {
      const canonicalSubject = visualArtsSubjects.find((entry) => entry.subjectId === subject.subjectId);
      expect(canonicalSubject).toBeDefined();
      ...
      const correctAnswerSequence: string[] = [];
      const wrongRationales: string[] = [];
      for (const [questionIndex, question] of subject.questions.entries()) {
        const canonicalFact = canonicalSubject!.facts[questionIndex];
        ...
        expect(correctOption?.text).toBe(canonicalFact.correct);
        expect(correctOption?.rationale).toContain(canonicalFact.explanation);
        for (const [wrongIndex, wrongText] of canonicalFact.wrong.entries()) {
          expect(question.options.find((option) => option.text === wrongText)?.rationale)
            .toBe(canonicalFact.wrongRationales[wrongIndex]);
        }
        correctAnswerSequence.push(question.correctAnswer);
        wrongRationales.push(...question.options
      ...
      expect(correctAnswerSequence.join('')).not.toBe('ABCD'.repeat(10));
      expect(correctAnswerSequence.join('')).not.toMatch(/AAAA|BBBB|CCCC|DDDD/);
      for (const label of ['A', 'B', 'C', 'D']) {
        const count = correctAnswerSequence.filter((answer) => answer === label).length;
        expect(count).toBeGreaterThanOrEqual(5);
        expect(count).toBeLessThanOrEqual(15);
      }
      expect(new Set(wrongRationales).size).toBe(wrongRationales.length);
      expect(wrongRationales.every((rationale) => !rationale.includes('credible same-domain alternative'))).toBe(true);
    }
  ...
  it('loads and reapplies 120 relationship-valid questions with exact beta provenance', () => {
    ...
    expect(db.prepare(`
      ...
      WHERE id LIKE 'q_was_%_b004_%' GROUP BY subject_id ORDER BY subject_id
    `).all()).toEqual([
      { subjectId: 'subj_wassce_basketry', count: 40 },
      { subjectId: 'subj_wassce_leatherwork', count: 40 },
      { subjectId: 'subj_wassce_sculpture', count: 40 },
    ]);
    expect(db.prepare(`
      ...
      WHERE q.id LIKE 'q_was_%_b004_%' AND q.subject_id = t.subject_id
        ...
        AND r.batch_id = 'wassce-visual-arts-beta-004' AND r.quality_assurance = 'automated_beta'
    ...
    applyAll(db);
    expect(db.prepare("SELECT COUNT(*) AS count FROM question_content_releases WHERE batch_id = 'wassce-visual-arts-beta-004'").get()).toEqual({ count: 120 });
    db.close();
  });
  ...
  it('fails closed on a stable-ID collision with different canonical content', () => {
    ...
    db.prepare(`INSERT INTO questions (
      ...
      .run('q_was_basketry_b004_001', 'topic_was_basketry_b004_1', 'subj_wassce_basketry', 'exam_wassce',
        'Corrupted content under a reserved visual-arts batch identifier', 'multiple_choice',
        ...
        'easy', 1, 1, 90, 'st_was_basketry_b004_1', 'Identify', 'AO1', 'board_waec');
  ...
  it('fails the final guard for missing parts or altered provenance', () => {
    ...
    applyAll(altered);
    altered.prepare("UPDATE question_content_releases SET official_exam_board_content = 1 WHERE question_id = 'q_was_basketry_b004_001'").run();
