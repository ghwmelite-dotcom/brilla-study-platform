...
function makeStrictDb(recordedSql: string[], opts: StubOptions = {}) {
  ...
  const first = vi.fn().mockImplementation(async function (this: { sql: string }) {
    const sql = this.sql;
    ...
    if (sql.includes('FROM users')) {
      return { id: 'student_1', name: 'Ama', email: 'ama@example.com', school_level: 'SHS', year_group: 2 };
    }
    return null;
  });
  ...
  const prepare = vi.fn().mockImplementation((sql: string) => {
    ...
    if (sql.includes('user_streaks')) {
      ...
      stmt.first = vi.fn().mockRejectedValue(new Error('no such table: user_streaks'));
      stmt.all = vi.fn().mockRejectedValue(new Error('no such table: user_streaks'));
      stmt.run = vi.fn().mockRejectedValue(new Error('no such table: user_streaks'));
    } else {
  ...
  it('POST /reports/generate succeeds and never queries user_streaks', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        content: [{
          type: 'text',
          text: '{"summary":"ok","academicPerformance":{},"wellbeingAssessment":{},' +
