...
const migrationNames = [
  '442_wassce_core_math_sprint2_foundation.sql',
  '443_wassce_core_math_sprint2_part_1.sql',
  '444_wassce_core_math_sprint2_part_2.sql',
  '445_wassce_core_math_sprint2_part_3.sql',
  '446_wassce_core_math_sprint2_part_4.sql',
  '447_wassce_core_math_sprint2_part_5.sql',
  '448_wassce_core_math_sprint2_part_6.sql',
  '449_wassce_core_math_sprint2_part_7.sql',
  '450_wassce_core_math_sprint2_part_8.sql',
  '451_wassce_core_math_sprint2_part_9.sql',
  '452_wassce_core_math_sprint2_part_10.sql',
  '453_wassce_core_math_sprint2_final_guard.sql',
] as const;

const repoRoot = fileURLToPath(new URL('../../../', import.meta.url));
const migrationRelativePaths = migrationNames.map((name) => `database/migrations/${name}`);
const artifactRelativePaths = ['content/batches/wassce-core-math-sprint2-001.json', ...migrationRelativePaths];
...
const seededTopics: Array<[string, string]> = [
  ...
  ['topic_wassce_p2_math_men', 'subj_wassce_core_math'],
  ['topic_wassce_p2_math_rat', 'subj_wassce_core_math'],
  ['topic_wassce_p2_math_prb', 'subj_wassce_core_math'],
];
...
describe('WASSCE Core Mathematics sprint 2 automated-beta migrations', () => {
  ...
  it('reproduces every committed JSON and SQL artifact byte for byte', () => {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'brilla-wassce-core-math-sprint2-'));
    try {
      execFileSync(process.execPath, [
        join(repoRoot, 'scripts/generate-wassce-core-math-sprint2-bank.mjs'),
        '--output-root',
        temporaryRoot,
  ...
  it('loads 48 original questions across nine prod-canonical topics with beta provenance', () => {
    const db = createFixture();
    applyAll(db);

    expect(db.prepare(`
      SELECT COUNT(*) AS count FROM questions WHERE id LIKE 'q_was_coremath_s2_%'
    `).get()).toEqual({ count: 48 });

    expect(db.prepare(`
      SELECT topic_id AS topicId, COUNT(*) AS count
      FROM questions WHERE id LIKE 'q_was_coremath_s2_%'
      GROUP BY topic_id ORDER BY topic_id
    `).all()).toEqual([
      { topicId: 'topic_wassce_p2_math_men', count: 8 },
      { topicId: 'topic_wassce_p2_math_num', count: 4 },
      { topicId: 'topic_wassce_p2_math_prb', count: 8 },
      { topicId: 'topic_wassce_p2_math_rat', count: 8 },
      { topicId: 'topic_wcoremath_algebra', count: 4 },
      { topicId: 'topic_wcoremath_geometry', count: 4 },
      { topicId: 'topic_wcoremath_quadratic', count: 4 },
      { topicId: 'topic_wcoremath_statistics', count: 4 },
      { topicId: 'topic_wcoremath_trigonometry', count: 4 },
    ]);
    ...
    `).all()).toEqual([
      ...
      { questionType: 'multiple_choice', count: 36 },
    ]);
    ...
    expect(db.prepare(`
      ...
      FROM questions WHERE id LIKE 'q_was_coremath_s2_%' AND question_type = 'multiple_choice'
      GROUP BY correct_answer ORDER BY correct_answer
    `).all()).toEqual([
      { correctAnswer: 'A', count: 9 },
      { correctAnswer: 'B', count: 9 },
      { correctAnswer: 'C', count: 9 },
      { correctAnswer: 'D', count: 9 },
    ]);
    ...
    expect(db.prepare(`
      ...
      WHERE q.id LIKE 'q_was_coremath_s2_%'
        ...
        AND qcr.batch_id = 'wassce-core-math-sprint2-001'
    ...
    expect(db.prepare(`
      ...
      WHERE batch_id = 'wassce-core-math-sprint2-001'
    ...
    applyAll(db);
    expect(db.prepare("SELECT COUNT(*) AS count FROM questions WHERE id LIKE 'q_was_coremath_s2_%'").get()).toEqual({ count: 48 });
    expect(db.prepare("SELECT COUNT(*) AS count FROM question_content_releases WHERE batch_id = 'wassce-core-math-sprint2-001'").get()).toEqual({ count: 48 });
    expect(db.pragma('foreign_key_check')).toEqual([]);
    db.close();
  });
  ...
  it('fails closed when an expected stable question ID contains different canonical content', () => {
    ...
    db.prepare(`
      ...
      VALUES ('q_was_coremath_s2_001', 'topic_wcoremath_algebra', 'subj_wassce_core_math', 'exam_wassce',
        'Corrupted content under a reserved stable ID', 'multiple_choice', 'fixture', 'easy')
    `).run();
    expect(() => db.exec(migrations[1])).toThrow();
    expect(db.prepare('SELECT question_text AS text FROM questions WHERE id = ?').get('q_was_coremath_s2_001')).toEqual({
  ...
  it('fails closed when a canonical row acquires false official-paper metadata', () => {
    ...
    db.prepare("UPDATE questions SET source_paper_code = 'WASSCE-2024', source_question_number = '1' WHERE id = 'q_was_coremath_s2_001'").run();
    expect(() => db.exec(migrations[1])).toThrow();
    db.close();
  });
  ...
  it('fails closed when existing release provenance differs', () => {
    ...
    db.exec(migrations[0]);
    db.exec(migrations[1]);
    db.prepare("UPDATE question_content_releases SET batch_id = 'wrong-batch' WHERE question_id = 'q_was_coremath_s2_001'").run();
    expect(() => db.exec(migrations[1])).toThrow();
    db.close();
  });
  ...
  it('fails closed when a reserved ID is pre-bound to the wrong topic of the same subject', () => {
    ...
    db.prepare(`
      INSERT INTO questions (id, topic_id, subject_id, exam_type_id, question_text, question_type, correct_answer, difficulty)
      VALUES ('q_was_coremath_s2_001', 'topic_wcoremath_quadratic', 'subj_wassce_core_math', 'exam_wassce',
        'Solve the equation 2x + 3 = x + 9.', 'multiple_choice', 'A', 'medium')
    `).run();
    expect(() => db.exec(migrations[1])).toThrow();
    db.close();
  });
  ...
  it('fails the final guard when earlier question parts are missing', () => {
    ...
    expect(db.prepare("SELECT COUNT(*) AS count FROM questions WHERE id LIKE 'q_was_coremath_s2_%'").get()).toEqual({ count: 0 });
    db.close();
  });
});
