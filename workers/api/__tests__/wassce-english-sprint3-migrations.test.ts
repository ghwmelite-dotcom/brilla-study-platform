...
const migrationNames = [
  '570_wassce_english_sprint3_foundation.sql',
  '571_wassce_english_sprint3_part_1.sql',
  '572_wassce_english_sprint3_part_2.sql',
  '573_wassce_english_sprint3_part_3.sql',
  '574_wassce_english_sprint3_part_4.sql',
  '575_wassce_english_sprint3_part_5.sql',
  '576_wassce_english_sprint3_part_6.sql',
  '577_wassce_english_sprint3_part_7.sql',
  '578_wassce_english_sprint3_part_8.sql',
  '579_wassce_english_sprint3_part_9.sql',
  '580_wassce_english_sprint3_final_guard.sql',
] as const;

const repoRoot = fileURLToPath(new URL('../../../', import.meta.url));
const migrationRelativePaths = migrationNames.map((name) => `database/migrations/${name}`);
const artifactRelativePaths = ['content/batches/wassce-english-sprint3-001.json', ...migrationRelativePaths];
...
// does on prod and on fresh baselines (prod patch 096 + migrations 363/422).
const seededTopics: Array<[string, string]> = [
  ['topic_wassce_p2_eng_cmp', 'subj_wassce_english'],
  ['topic_wassce_p2_eng_ess', 'subj_wassce_english'],
  ['topic_wassce_p2_eng_sum', 'subj_wassce_english'],
  ['topic_wassce_english_summary', 'subj_wassce_english'],
  ['topic_wassce_english_essay', 'subj_wassce_english'],
  ['topic_wassce_english_letter', 'subj_wassce_english'],
...
describe('WASSCE English Language sprint 3 automated-beta migrations', () => {
  ...
  it('reproduces every committed JSON and SQL artifact byte for byte', () => {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'brilla-wassce-english-sprint3-'));
    try {
      execFileSync(process.execPath, [
        join(repoRoot, 'scripts/generate-wassce-english-sprint3-bank.mjs'),
        '--output-root',
        temporaryRoot,
  ...
  it('loads 40 original questions across ten prod-canonical topics with beta provenance', () => {
    const db = createFixture();
    applyAll(db);

    expect(db.prepare(`
      SELECT COUNT(*) AS count FROM questions WHERE id LIKE 'q_eng_s3_%'
    `).get()).toEqual({ count: 40 });

    expect(db.prepare(`
      SELECT topic_id AS topicId, COUNT(*) AS count
      FROM questions WHERE id LIKE 'q_eng_s3_%'
      GROUP BY topic_id ORDER BY topic_id
    `).all()).toEqual([
      { topicId: 'topic_wassce_english_essay', count: 1 },
      { topicId: 'topic_wassce_english_grammar', count: 1 },
      { topicId: 'topic_wassce_english_letter', count: 1 },
      { topicId: 'topic_wassce_english_lexis', count: 1 },
      { topicId: 'topic_wassce_english_literary_devices', count: 1 },
      { topicId: 'topic_wassce_english_oral', count: 1 },
      { topicId: 'topic_wassce_english_summary', count: 3 },
      { topicId: 'topic_wassce_p2_eng_cmp', count: 10 },
      { topicId: 'topic_wassce_p2_eng_ess', count: 11 },
      { topicId: 'topic_wassce_p2_eng_sum', count: 10 },
    ]);
    ...
    `).all()).toEqual([
      { questionType: 'multiple_choice', count: 33 },
    ...
    expect(db.prepare(`
      ...
      FROM questions WHERE id LIKE 'q_eng_s3_%' AND question_type = 'multiple_choice'
      GROUP BY correct_answer ORDER BY correct_answer
    `).all()).toEqual([
      { correctAnswer: 'A', count: 9 },
      { correctAnswer: 'B', count: 8 },
      { correctAnswer: 'C', count: 8 },
      { correctAnswer: 'D', count: 8 },
    ]);

    expect(db.prepare(`
      SELECT COUNT(*) AS count FROM structured_question_parts
      WHERE question_id LIKE 'q_eng_s3_%'
      ...
      WHERE q.id LIKE 'q_eng_s3_%'
        ...
        AND (
          (q.question_type = 'multiple_choice' AND json_valid(q.options) AND json_array_length(q.options) = 4
            AND q.correct_answer IN ('A', 'B', 'C', 'D') AND q.points = 3 AND q.marks = 1 AND q.time_limit IN (45, 60))
          OR
          (q.question_type = 'short_answer' AND q.options IS NULL AND length(q.correct_answer) >= 1
            AND q.points = 4 AND q.marks = 2 AND q.time_limit = 90)
          OR
          (q.question_type = 'structured' AND q.options IS NULL
            AND q.points = q.marks AND q.marks IN (6, 10) AND q.time_limit = 300)
        )
        AND qcr.batch_id = 'wassce-english-sprint3-001'
    ...
    expect(db.prepare(`
      ...
      WHERE batch_id = 'wassce-english-sprint3-001'
    ...
    applyAll(db);
    expect(db.prepare("SELECT COUNT(*) AS count FROM questions WHERE id LIKE 'q_eng_s3_%'").get()).toEqual({ count: 40 });
    expect(db.prepare("SELECT COUNT(*) AS count FROM question_content_releases WHERE batch_id = 'wassce-english-sprint3-001'").get()).toEqual({ count: 40 });
    expect(db.pragma('foreign_key_check')).toEqual([]);
    db.close();
  });
  ...
  it('fails closed when an expected stable question ID contains different canonical content', () => {
    ...
    // Use the first question of part 1 so the tamper lands in a part the test applies.
    const partOneFirstId = allQuestionIds.find((id) => migrations[1].includes(`'${id}'`))!;
    db.prepare(`
      INSERT INTO questions (id, topic_id, subject_id, exam_type_id, question_text, question_type, correct_answer, difficulty)
      VALUES (?, 'topic_wassce_p2_eng_cmp', 'subj_wassce_english', 'exam_wassce',
        'Corrupted content under a reserved stable ID', 'multiple_choice', 'fixture', 'easy')
    `).run(partOneFirstId);
    expect(() => db.exec(migrations[1])).toThrow();
    expect(db.prepare('SELECT question_text AS text FROM questions WHERE id = ?').get(partOneFirstId)).toEqual({
  ...
  it('fails closed when a canonical row acquires false official-paper metadata', () => {
    ...
    const partOneFirstId = allQuestionIds.find((id) => migrations[1].includes(`'${id}'`))!;
    db.prepare("UPDATE questions SET source_paper_code = 'WASSCE-2024', source_question_number = '1' WHERE id = ?").run(partOneFirstId);
    expect(() => db.exec(migrations[1])).toThrow();
    db.close();
  });
  ...
  it('fails closed when existing release provenance differs', () => {
    ...
    db.exec(migrations[1]);
    const partOneFirstId = allQuestionIds.find((id) => migrations[1].includes(`'${id}'`))!;
    db.prepare("UPDATE question_content_releases SET batch_id = 'wrong-batch' WHERE question_id = ?").run(partOneFirstId);
    expect(() => db.exec(migrations[1])).toThrow();
    db.close();
  });
  ...
  it('fails closed when a reserved ID is pre-bound to the wrong topic of the same subject', () => {
    ...
    db.prepare(`
      INSERT INTO questions (id, topic_id, subject_id, exam_type_id, question_text, question_type, correct_answer, difficulty)
      VALUES (?, 'topic_wassce_p2_eng_ess', 'subj_wassce_english', 'exam_wassce',
        'An essay written to explain a process or give information is called', 'multiple_choice', 'A', 'easy')
    `).run(partOneFirstId);
    expect(() => db.exec(migrations[1])).toThrow();
    db.close();
  });
  ...
  it('rejects cross-subject topic bindings through the subject/topic consistency trigger', () => {
    ...
    expect(() => db.prepare(`
      INSERT INTO questions (id, topic_id, subject_id, exam_type_id, question_text, question_type, correct_answer, difficulty)
      VALUES ('q_scratch_001', 'topic_algebra', 'subj_wassce_english', 'exam_wassce',
  ...
  it('fails the final guard when earlier question parts are missing', () => {
    ...
    expect(db.prepare("SELECT COUNT(*) AS count FROM questions WHERE id LIKE 'q_eng_s3_%'").get()).toEqual({ count: 0 });
    db.close();
  });
});
