...
function makeDb({ adminCount = 0, existingEmails = [], rateLimitCount = 0 }: DbOptions = {}) {
...
const VALID_BODY = {
  setupKey: SETUP_KEY,
  users: [
    { email: 'teacher@brillaprep.org', password: 'S3cure!Pass', name: 'Demo Teacher', role: 'teacher' },
  ],
};
...
describe('/auth/setup hardening', () => {
  it('returns 404 when SETUP_KEY is not configured (endpoint disabled)', async () => {
    const { db } = makeDb();
    const res = await worker.fetch(setupRequest(VALID_BODY), { DB: db, JWT_SECRET });
  ...
  it('returns 403 when an admin already exists (one-shot guard)', async () => {
    ...
    const res = await worker.fetch(setupRequest(VALID_BODY), { DB: db, JWT_SECRET, SETUP_KEY });
    expect(res.status).toBe(403);
    expect(await res.json()).toMatchObject({ error: 'Setup has already been completed' });
  });
  ...
  it("returns 400 when the users array contains role: 'admin'", async () => {
    ...
    const res = await worker.fetch(
      setupRequest({
        setupKey: SETUP_KEY,
        users: [{ email: 'a@b.com', password: 'S3cure!Pass', name: 'Bad', role: 'admin' }],
      }),
      { DB: db, JWT_SECRET, SETUP_KEY },
    );
  ...
  it('returns 400 when no users array is supplied (no default credentials)', async () => {
    const { db } = makeDb();
    const res = await worker.fetch(
      setupRequest({ setupKey: SETUP_KEY }),
      { DB: db, JWT_SECRET, SETUP_KEY },
    );
    expect(res.status).toBe(400);
  });
  ...
  it('creates users and skips existing emails without overwriting passwords', async () => {
    ...
    const res = await worker.fetch(
      setupRequest({
        setupKey: SETUP_KEY,
        users: [
          { email: 'teacher@brillaprep.org', password: 'S3cure!Pass', name: 'T', role: 'teacher' },
          { email: 'student@brillaprep.org', password: 'S3cure!Pass', name: 'S', role: 'student' },
        ],
      }),
      { DB: db, JWT_SECRET, SETUP_KEY },
    );
