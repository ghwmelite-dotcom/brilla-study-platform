...
// Middleware to check authentication and admin/moderator role
moderationApp.use('*', async (c, next) => {
...
moderationApp.post('/reports', async (c) => {
  ...
  if (!body.reported_user_id || !body.reason) {
    return c.json({ error: 'reported_user_id and reason are required' }, 400);
  }
  ...
  if (existingReport) {
    return c.json({ error: 'You have already reported this user recently' }, 400);
  }
...
moderationApp.post('/actions/mute', async (c) => {
  ...
  if (!body.user_id || !body.reason || !body.duration) {
    return c.json({ error: 'user_id, reason, and duration are required' }, 400);
  }
...
moderationApp.post('/actions/kick', async (c) => {
  ...
  if (!body.user_id || !body.room_id || !body.reason) {
    return c.json({ error: 'user_id, room_id, and reason are required' }, 400);
  }
...
moderationApp.post('/filters', async (c) => {
  ...
  try {
    ...
    if (e.message?.includes('UNIQUE constraint failed')) {
      return c.json({ error: 'Word already exists in filter list' }, 400);
    }
    throw e;
  }
