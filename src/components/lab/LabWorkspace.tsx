...
export function LabWorkspace({ onExit }: LabWorkspaceProps) {
  ...
  // Reveal the data panel when it opens — on short screens the workspace
  // root scrolls, so the panel can otherwise land below the visible area.
  useEffect(() => {
    if (showDataPanel) {
      dataPanelRef.current?.scrollIntoView?.({ block: 'nearest', behavior: 'smooth' });
    }
  }, [showDataPanel]);
    ...
    <div className={cn(
      ...
      <div className="relative z-10 flex flex-1">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          {/* Data Recording Panel — expands upward above the nav bar.
              Lives inside the lab-area column so the sim canvas shrinks to
              make room; scrollIntoView covers short screens where the
              column overflows and the root scrolls. */}
          {showDataPanel && (
            <div ref={dataPanelRef} className={cn(
