...
function mapTable(name, rows) {
  ...
  return `CREATE TABLE ${name}(question_id TEXT PRIMARY KEY,topic_id TEXT NOT NULL);${[...groups].map(([topicId, questionIds]) => `INSERT INTO ${name} SELECT column1,${sql(topicId)} FROM(VALUES${questionIds.map((questionId) => `(${sql(questionId)})`).join(',')});`).join('')}`;
}
...
const scopeTable = () => `CREATE TABLE _sr (${SCOPE_SCHEMA});INSERT INTO _sr (${SCOPE_COLUMNS}) SELECT ${SCOPE_COLUMNS} FROM questions WHERE ${SCOPE_SQL};`;
function topicSpecsSql() { return `CREATE TABLE _ts(id TEXT PRIMARY KEY,subject_id TEXT NOT NULL,name TEXT NOT NULL,slug TEXT NOT NULL,description TEXT NOT NULL,display_order INTEGER NOT NULL,created_at TEXT NOT NULL);INSERT INTO _ts VALUES${NEW_TOPICS.map((topic) => `(${[topic.id,topic.subjectId,topic.name,topic.slug,topic.description,topic.displayOrder,topic.createdAt].map(sql).join(',')})`).join(',')};`; }
...
module.exports={buildArtifacts,buildModel,fingerprint,rowsFingerprint,fixture};
